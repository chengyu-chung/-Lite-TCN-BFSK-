...
//    V7 协议：sync(8)+dest(4)+src(4)+payload(16)=32bit
// ====================================================================
#define FRAME_BITS       32   // 单帧有效比特数
#define TAIL_BITS        4    // 发射尾部补零位（保护继电器切换）
#define FRAME_SAMPLES    960  // AI 输入采样点数（与模型 input width 一致）
...
typedef struct {
  ...
  //  uint8_t   syncMatched;         // 同步头匹配标志
    ...
    uint32_t  payloadData;         // 载荷缓存（V7: payload16=[temp8|pres8]，有效位为低16bit）
...
#define AD9833FSYNG_Pin GPIO_PIN_4
#define AD9833FSYNG_GPIO_Port GPIOA
...
// 传感器使用开关：0=使用内置模拟温压数据，1=使用真实 DS18B20 传感器
...
// --- 本机身份配置 ---
#define LOCAL_ADDRESS    ADDR_UNDERGROUND // 当前设备的绝对物理地址
